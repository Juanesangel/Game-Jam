...
class EscenaCinematica:
    ...
    def __init__(self, cambiar_escena_cb, ancho, alto,
        ...
        if ruta_fondo:
            try:
                ...
                print(f"[AVISO] No se pudo cargar fondo de cinemática: {e}")
        ...
        if ruta_abuela:
            try:
                ...
                print(f"[AVISO] No se pudo cargar retrato de abuela: {e}")

        # Área de texto (a la derecha del retrato si existe)
        if self.retrato:
        ...
        # Diálogos de ejemplo (luego los reemplazas)
        self.dialogos = [
            {"speaker": "Abuela ", "text": "Hola, mi niño, como estas?."},
            {"speaker": "Tú ",     "text": "Abuela... Muy bien y tu como estas? ¿Que haces por aqui?."},
            {"speaker": "Abuela ", "text": "Ayyy mi niño!... La vida se ha vuelto un poco dura, y me ha tocado sobrevivir aqui! Vendiendo empanadas"},
            {"speaker": "Tú ",     "text": "Ay abuelita... Yo te podria ayudar!"},
            {"speaker": "Abuela ", "text": "Ayy mi niño, no sabria como pagarte!"},
            {"speaker": "Tú ", "text": "No te preocupes abuelita. Juntos saldremos adelante!"}
        ]

        # Estado de la narrativa
        self.idx = 0
        self.cps = cps  # caracteres por segundo
    ...
    def dibujar(self, surface):
        ...
        # Caja de diálogo (panel semitransparente)
        ...
        if self.idx < len(self.dialogos):
            ...
            # Esquina superior izquierda de la caja de diálogo
            ...
            # Texto (con wrap), solo la parte visible
...
if __name__ == "__main__":
    ...
    # Elige uno:
    # 1) Tamaño fijo de prueba:
    ANCHO, ALTO = min(1280, info.current_w), min(720, info.current_h)
    VENTANA = pygame.display.set_mode((ANCHO, ALTO))
    # 2) O pantalla completa (descomenta la siguiente línea y comenta la anterior):
    ...
    # Crea la escena de cinemática.
    # Ajusta las rutas si tienes un fondo o el retrato de la abuela.
    try:
        escena = EscenaCinematica(
            cambiar_escena_cb=dummy_cambiar_escena,
            ancho=ANCHO,
            alto=ALTO,
            ruta_fondo="assets/Assets_Menu_inicio/Conversacion.png",  # opcional; comenta si no la tienes
            ruta_abuela="assets/cinematics/abuela.png",                    # opcional; comenta si no la tienes
            cps=40  # velocidad del efecto máquina de escribir (caracteres/segundo)
        )
    except Exception as e:
    ...
    while True:
        ...
        # Atajo rápido para salir del preview con ESC aunque no lo maneje la escena
        for ev in eventos:
            if ev.type == pygame.QUIT:
                pygame.quit()
                sys.exit()
            if ev.type == pygame.KEYDOWN and ev.key == pygame.K_q:
                print("[Preview] Saliendo con Q")
                pygame.quit()
                sys.exit()
